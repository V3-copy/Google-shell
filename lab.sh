...
echo -e "${GREEN}[*] Configuring root authentication & installing security tools...${NC}"
docker exec -u 0 cyber-lab bash -c "
    echo 'root:12345678' | chpasswd && \
    echo 'kasm-user:12345678' | chpasswd && \
    (echo '12345678'; echo '12345678') | kasmvncpasswd -u root -w /home/kasm-user/.kasmpasswd && \
    chmod 644 /home/kasm-user/.kasmpasswd && \
    chown kasm-user:kasm-user /home/kasm-user/.kasmpasswd && \
...
if [ -z "$TUNNEL_URL" ]; then
    ...
    echo -e "💡 Note           : Passwordless sudo enabled in GUI terminal"
    echo ""
    ...
    # Flush residual key presses
    while read -e -t 0.1 -n 10000 discard; do : ; done 2>/dev/null
    
    # Wait for intentional key press to stop
    read -n 1 -s -r
    cleanup
fi
